-- Corrigir função verificar_inscricao_semanal para resolver erro de operação entre date e numeric
...
BEGIN
    ...
    SELECT 
      CASE 
        WHEN dia_semana >= EXTRACT(dow FROM CURRENT_DATE) THEN
          CURRENT_DATE + INTERVAL '1 day' * (dia_semana - EXTRACT(dow FROM CURRENT_DATE))
        ELSE
          CURRENT_DATE + INTERVAL '1 day' * (dia_semana - EXTRACT(dow FROM CURRENT_DATE) + 7)
  ...
  inicio_semana := data_aula_nova - INTERVAL '1 day' * EXTRACT(dow FROM data_aula_nova);
  fim_semana := inicio_semana + INTERVAL '6 days';
      ...
      (a.data_aula IS NULL AND 
        CASE 
          WHEN a.dia_semana >= EXTRACT(dow FROM CURRENT_DATE) THEN
            CURRENT_DATE + INTERVAL '1 day' * (a.dia_semana - EXTRACT(dow FROM CURRENT_DATE))
          ELSE
            CURRENT_DATE + INTERVAL '1 day' * (a.dia_semana - EXTRACT(dow FROM CURRENT_DATE) + 7)
